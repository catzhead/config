...
__powerline() {
    ...
    COLOR_GIT=${COLOR_GIT:-'\[\033[0;36m\]'} # cyan
		COLOR_VENV=${COLOR_VENV:-'\[\033[0;33m\]'} # yellow
    ...
    ps1() {
        ...
        if shopt -q promptvars; then
            ...
            local git="$COLOR_GIT$(__git_info)$COLOR_RESET"
				...
				local VENV_CLEAN="$(echo -e "${VIRTUAL_ENV_PROMPT}" | sed -e 's/[[:space:]]*$//')"
				VENV_CLEAN="$(echo -e "$VENV_CLEAN" | sed -e 's/(//')"
				VENV_CLEAN="$(echo -e "$VENV_CLEAN" | sed -e 's/)//')"
